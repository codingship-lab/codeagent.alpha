...
function createAutoLandingPatch(messages) {
  ...
  const heroTitle =
    /пицц|pizza/i.test(userText)
      ...
      : /рыб|fish/i.test(userText)
        ? 'Свежая рыба из Норвегии'
        : 'Современный лендинг для продаж';
  const heroSubtitle = `Проект: ${userText || 'Landing page'}`;
  ...
  return [
    ...
    'const cards = [',
    "  { title: 'Быстрая доставка', text: 'Привозим за 30-45 минут в вашем районе.' },",
    "  { title: 'Свежие ингредиенты', text: 'Готовим из качественных продуктов каждый день.' },",
    "  { title: 'Онлайн-заказ', text: 'Оформляйте заказ за минуту с любого устройства.' }",
    '];',
    '',
    ...
    '          <button className="hero__cta">Заказать сейчас</button>',
    '        </div>',
    '      </header>',
    '',
...
- If source text is unclear, return final with concise message.`;
  ...
  async resolveReadablePath(requestedPath) {
    ...
    if (/^index\.html$/i.test(normalizedRequested)) {
      pushCandidate('public/index.html');
    }
    if (/^public$/i.test(normalizedRequested)) {
      pushCandidate('public/index.html');
    }
    if (/^src$/i.test(normalizedRequested)) {
      pushCandidate('src/main.tsx');
      pushCandidate('src/main.jsx');
      pushCandidate('src/App.tsx');
      pushCandidate('src/App.jsx');
    }
