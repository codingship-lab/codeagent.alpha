    ...
    <div className="h-screen bg-background flex flex-col overflow-hidden font-sans">
      <header className="flex items-center justify-between px-8 py-4 border-b border-border/50 bg-background/80 backdrop-blur-xl z-10">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate("/")}>
           ...
           <span className="text-xl font-black tracking-tighter hidden sm:inline text-foreground uppercase italic">CodeAgent</span>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
          <div className="flex items-center justify-between px-8 py-3">
            <TabsList className="bg-secondary border border-border h-11 p-1 rounded-full px-1">
              <TabsTrigger value="chat" className="gap-2 rounded-full px-8 data-[state=active]:bg-primary data-[state=active]:text-white font-bold uppercase text-xs tracking-widest transition-all">
                <MessageSquare className="h-4 w-4" />
                Chat
              </TabsTrigger>
              <TabsTrigger value="preview" className="gap-2 rounded-full px-8 data-[state=active]:bg-primary data-[state=active]:text-white font-bold uppercase text-xs tracking-widest transition-all">
                <Eye className="h-4 w-4" />
                Preview
              </TabsTrigger>
            </TabsList>

            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full text-muted-foreground hover:text-foreground hover:bg-muted/50">
          ...
          <TabsContent value="preview" className="flex-1 bg-white m-0 rounded-[40px] overflow-hidden shadow-2xl mx-6 mb-6 border-4 border-secondary">
